...
def load_and_process(filename):
    ...
    df2 = (
        df1.rename(columns={'diagnosis': 'Diagnosis', 'radius_mean': 'Mean Radius', 'texture_mean': 'Mean Texture', 'perimeter_mean': 'Mean Perimeter',
        ...
        .assign(Diagnosis=lambda x: x['Diagnosis'].map({'M': 'Malignant', 'B': 'Benign'}))
        .apply(lambda x: round(x, 3) if np.issubdtype(x.dtype, np.number) else x) 
    )
